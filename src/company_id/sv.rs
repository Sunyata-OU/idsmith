...
pub fn generate(rng: &mut rand::rngs::ThreadRng) -> String {
    ...
    digits[0] = *["0", "1", "9"][rng.gen_range(0..3)].as_bytes().first().unwrap() - b'0';
...
pub fn validate(code: &str) -> bool {
    ...
    if ![0, 1, 9].contains(&digits[0]) { return false; }
    
    let expected = if digits[10..13].iter().fold(0, |a, &b| a * 10 + b as u16) <= 100 {
        let weights = [14, 13, 12, 11, 10, 9, 8, 7, 6, 5, 4, 3, 2];
        let sum: u32 = digits[..13].iter().zip(weights.iter()).map(|(&d, &w)| d as u32 * w as u32).sum();
        (sum % 11) % 10
    } else {
        let weights = [2, 7, 6, 5, 4, 3, 2, 7, 6, 5, 4, 3, 2];
        let sum: u32 = digits[..13].iter().zip(weights.iter()).map(|(&d, &w)| d as u32 * w as u32).sum();
