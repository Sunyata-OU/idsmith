...
pub fn validate(code: &str) -> bool {
    ...
    if "IOU".contains(control) || !control.is_ascii_alphabetic() {
        return false;
    }
    if clean.len() == 13 {
        if !"ABCDE".contains(clean.chars().nth(8).unwrap()) {
            return false;
        }
        if !"MCPNE".contains(clean.chars().nth(9).unwrap()) {
            return false;
        }
        if !clean[10..].chars().all(|c| c.is_ascii_digit()) {
            return false;
        }
    }
    true
